...
test('Switch component with color props', () => {
    const switchComponent = enzyme_1.shallow(react_1.default.createElement(_1.Switch, { color: "primary" }));
    console.log(switchComponent.props, 'checked');
    expect(switchComponent.props().color).toEqual('primary');
});
test('Switch component with disabled props', () => {
    const switchComponent = enzyme_1.shallow(react_1.default.createElement(_1.Switch, { disabled: true }));
    console.log(switchComponent.props, 'checked');
    expect(switchComponent.props().disabled).toEqual(true);
});
test('Switch component with disableRipple props', () => {
    const switchComponent = enzyme_1.shallow(react_1.default.createElement(_1.Switch, { disableRipple: false }));
    console.log(switchComponent.props, 'checked');
    expect(switchComponent.props().disableRipple).toEqual(false);
});
test('Switch component with checked props', () => {
    const switchComponent = enzyme_1.shallow(react_1.default.createElement(_1.Switch, { checkedIcon: true }));
    console.log(switchComponent.props, 'checked');
