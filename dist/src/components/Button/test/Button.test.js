...
test('Rounded Button should properly render', () => {
    const wrapper = enzyme_1.shallow(React.createElement(_1.Button, { rounded: false, style: { marginLeft: 150, display: 'flex', justifyContent: 'center', } }, "rounded"));
    expect(wrapper.props().rounded).toBeFalsy();
});
test('Button with fab prop small false', () => {
    const wrapper = enzyme_1.shallow(React.createElement(_1.Button, { block: true, transparent: true, style: { marginLeft: 150, display: 'flex', justifyContent: 'center', } }, "Transparent"));
    expect(wrapper).toBeDefined();
});
// // // import { Button } from '../';
